...
### TD-3.4 — `mathbot lint` subcommand not yet shipped

Phase 4 plan recommended a lint subcommand that flags non-anchor
templates whose math-step-count or variable-set diverges from the
anchor in their (grade, topic, family, difficulty) quadruplet. Would
also catch missing schema fields, off-anchor naming, etc.
...
`contamination.json`, `coverage.csv`, `sample200.jsonl`, `runs/` are
generated by [scripts/coverage_matrix.py](scripts/coverage_matrix.py),
[scripts/gsm8k_contamination.py](scripts/gsm8k_contamination.py), and
[scripts/internal_contamination.py](scripts/internal_contamination.py).
They land in the working tree as untracked noise.
...
**Note**: the entry that previously listed `scripts/_eval/`,
`scripts/_ollama.py`, and `scripts/test_model.py` here is obsolete
— those files were the smoke harness that was retired in v0.2.1
when mathbot became a gol_eval plugin. The remaining utility
scripts (`coverage_matrix.py`, `gsm8k_contamination.py`,
`internal_contamination.py`, `refresh_test_answers.py`) are
committed.
