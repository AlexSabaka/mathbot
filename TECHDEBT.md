...
### TD-5.5 — Untracked utility scripts in `scripts/`

`scripts/_eval/`, `scripts/_ollama.py`, `scripts/coverage_matrix.py`,
`scripts/gsm8k_contamination.py`, `scripts/test_model.py` are untracked
in git but referenced from various plan/proposal docs. Same for
`contamination.json`, `coverage.csv`, `sample200.jsonl`, `runs/`.

**Resolution**: User decision — commit the scripts (and add a
`scripts/README.md` describing what each does), or `.gitignore` the data
artefacts. The data files (`contamination.json`, `sample200.jsonl`,
`runs/`) are large and look like ephemeral run output.
